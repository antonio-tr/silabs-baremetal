...
void display_init(void)
{
  ...
  status = DMD_init(0);
  if (status != DMD_OK) {
    printf("[DMD] - Error 0x%lx initializing DMD driver!", status);
    while (1) {
    }
  }
  ...
  if (status != GLIB_OK) {
    printf("[GLIB] - Error 0x%lx initializing GLIB context!", status);
    while (1) {
    }
  }
}
...
void display_rht_screen(void)
{
  ...
  if (status != GLIB_OK) {
    printf("[GLIB] - Error 0x%lx clearing display!", status);
    while (1) {
    }
  }
  ...
  strncpy(str, "RHT MCU example", sizeof("RHT MCU example"));
  ...
  sprintf(str, "T:%2.1f C", 0);
  status = GLIB_drawString(&glib_context,
                      str,
                      strlen(str),
                      ((LCD_CENTER_X/2) - ((LCD_FONT_WIDTH * strlen(str)) / 2)),
                      (LCD_MAX_Y - 5 - (LCD_FONT_HEIGHT/2)),
                      true);

  sprintf(str, "RH:%2.1f %%", 0);
  status = GLIB_drawString(&glib_context,
                           str,
                           strlen(str),
                           (LCD_CENTER_X + (LCD_CENTER_X/2) - \
                           ((LCD_FONT_WIDTH * strlen(str)) / 2)),
                           (LCD_MAX_Y - 5 - (LCD_FONT_HEIGHT/2)),
                           true);

  if (status != GLIB_OK) {
    printf("[GLIB] - Error 0x%lx drawing to display!", status);
    while (1) {
    }
  }
  ...
  if (status != DMD_OK) {
    printf("[DMD] - Error 0x%lx updating display!", status);
    while (1) {
    }
  }
}
...
void display_rht_screen_update(float rh_data, float temp_data)
{
  ...
  if (status != GLIB_OK) {
    printf("[GLIB] - Error 0x%lx setting clipping region!", status);
    while (1) {
    }
  }
  ...
  if (status != GLIB_OK) {
    printf("[GLIB] - Error 0x%lx clearing region!", status);
    while (1) {
    }
  }
  ...
  if (status != GLIB_OK) {
    printf("[GLIB] - Error 0x%lx resetting clipping region!", status);
    while (1) {
    }
  }
  ...
  if (status != GLIB_OK) {
    printf("[GLIB] - Error 0x%lx applying clipping region!", status);
    while (1) {
    }
  }
  ...
  // Modify font size and draw temperature and RH data
  glib_context.font = GLIB_FontNarrow6x8;
  sprintf(str, "T:%2.1f C", temp_data);
  status = GLIB_drawString(&glib_context,
                      str,
                      strlen(str),
                      ((LCD_CENTER_X/2) - ((LCD_FONT_WIDTH * strlen(str)) / 2)),
